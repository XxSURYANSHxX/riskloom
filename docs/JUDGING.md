...
| Build quality | A typed FastAPI modular monolith with PostgreSQL, append-only audit records, causal state, portable JSON inference, deterministic graph layout, pinned runtime artifacts, and fail-closed startup. | [Architecture](../README.md#how-it-works); [`FeatureEngine`](../src/riskloom/features/engine.py); [`preflight.py`](../src/riskloom/services/preflight.py); [`runtime_bundle.py`](../src/riskloom/runtime_bundle.py); [current verification record](BUILD_LOG.md#current-verified-state) |
